...
const state = reactive({
  // Title of the current url, derived from constants.js, e.g. Online Timer, Classroom Timer
  appTitle: null,
 ...
 */
const cacheKeyBackgroundImageFileSelected = `${utility.cacheKeyPrefix}__backgroundImageFileSelected`;
...
const backgroundImageFileSelectedUrl = computed(() => {
  ...
  const result = new URL(`/src/assets/images/${fileName}`, import.meta.url).href;
 ...
 * @param {int} value
 */
const setInitialTimer = (valueTimer, isInitialTimerUpdatable = true) => {
  state.initialTimer = valueTimer;

  state.isInitialTimerUpdatable = isInitialTimerUpdatable;
};

const setPageMetaDescription = (metaDescription) => {
